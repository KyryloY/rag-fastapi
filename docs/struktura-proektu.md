...
`main.py` обрізає службові пробіли в питанні, перевіряє довжину, відкриває Chroma, дістає п'ять карток і лише потім кличе `answer_question`. Якщо модель відповіла по суті, у JSON їдуть `sources` з цих карток (заголовок статті й текст абзаца до чотирьох тисяч символів). Якщо `refused: true`, масив `sources` порожній: сусіди з пошуку лишаються в промпті моделі, але не показуються як джерела відповіді, якої немає.

## Вебінтерфейс і адреси

`GET /` віддає HTML-сторінку. На ній є дисклеймер, поле питання, кнопка, місце для відповіді й список джерел. Скрипт на сторінці надсилає `POST /ask` і малює результат. Заголовок «Quellen» з'являється лише коли в JSON є джерела. Кожне джерело - елемент `<details>`: у закритому вигляді видно підпис абзаца, після кліку розгортається цитата. Текст цитати вставляється як звичайний текст, не як HTML, щоб випадкові символи з закону не зламали сторінку.
...
`answer` і `refused` беруться з кроку 7. Якщо `refused` хибний, масив `sources` береться з карток кроку 5 (поле `text` обрізається до 4000 символів і називається `snippet`). Якщо `refused` істинний, `sources` порожній. Pydantic-клас `AskResponse` перевіряє форму. HTTP 200:
...
У живому прогоні при змістовній відповіді в `sources` п'ять елементів, якщо індекс не порожній. Тут показано три, щоб приклад лишався читабельним.
...
Клієнт отримує той самий `answer` і `refused: true`, а `sources` порожній. Пошук усе одно віддав п'ять сусідів у промпт, інакше модель не мала б на чому відмовити. У відповіді людині ці картки не показують: інакше мінімальна зарплата чи абзац про дохід виглядали б як джерела зарплати CEO.
